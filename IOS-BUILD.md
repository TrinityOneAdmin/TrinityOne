...
Android. The web app in `www/` is wrapped in a WKWebView. The iOS project was
scaffolded on Linux; the steps that require Apple tooling (CocoaPods, Xcode,
signing, archiving) **must be run on a Mac** — they cannot be done on Linux.

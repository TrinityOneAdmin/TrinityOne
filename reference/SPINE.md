...
### Censorship-resistant module distribution  *(built 2026-06-23; branch `claude/blossom`; not yet merged)*
Bible modules + the `catalog.json` listing them today fetch from one HTTP host (`ASSET_BASE`) — block that and members lose both list and bytes, even though Bible content is public-domain and chat is already decentralised across multiple relays. Two-phase plan, full design in `reference/proposal-blossom.md`. **Phase 1 — multi-mirror URLs + per-entry sha256** (~1 day, low-risk, ship mid-pilot): `catalog.json` entries gain `mirrors:[…]`; engine.js tries each in turn with sha256 verification (already wired — `KNOWN_HASHES`/`verifyIntegrity` in engine.js). Every church relay's gateway already serves `/modules/*`, so master-01, the NAS, and the dev box become mirrors with a list change, not new code. Covers single-host failure / casual blocking. **Phase 2 — Blossom + signed-Nostr catalog** (~5–7 days): blob server on every church relay (`GET /blossom/<sha256>` — looks like a plain HTTPS image fetch, no DPI signature); catalog republished as a signed `kind:30078` event from a stable "TrinityOne content key" so the *list* rides on any relay too. The pubkey is baked into the app; sha256 in the event is the authority, so neither a malicious mirror (wrong bytes → rejected) nor a malicious relay (wrong catalog → signature fails) can poison anything. Threat model: hostile state / ISP blocking the trinityone.church host — *not* content tampering (sha256 already covers that). **Out of scope here:** module *publishing* from the app, member-to-member WebRTC seeding, Tor for anonymising mirror connections (operator-coercion attack — noted, deferred). **Optional disaster-archive layer:** generate `.torrent` per module + magnet URIs in the catalog (~0.5 day). Not used by the app (torrents are mobile-hostile + state-DPI-blocked), but lets anyone recover the library if every relay/mirror went dark.

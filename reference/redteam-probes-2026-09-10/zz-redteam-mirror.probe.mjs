...
// Throwaway. Port 9112 — checked free at author time; not in the suite's fixed-port map.
...
(async () => {
  dataDir = mkdtempSync(join(tmpdir(), 'trin-rt-mirror-'));
  await boot();
  w = await conn();
  ...
  // A. THE SAFEGUARDING STEWARD clears Ada — the newly-widened mint, and the baseline.
  console.log('sgLead grant OK =', await publishAs(sgLead, permission(sgLead, church, ada, { at: now() - 60 })));
  await sleep(250);
  await step('A. steward cleared Ada (BASELINE — must be 1/1/true)');

  // B. Reboot with the roster unchanged: it must survive.
  await reboot();
  await step('B. after a reboot, roster unchanged (must be 1/1/true)');

  // C. The church takes safeguarding OFF that steward. No restart. Is the clearance still live?
  console.log('de-cap OK =', await publishAs(church, doc(church, D.STEWARDS + church.pub,
    { pubkeys: [sgLead.pub], caps: { [sgLead.pub]: ['members'] } })));
  await sleep(300);
  await step('C. steward de-capped, NO restart (grantorOk analogue?)');

  // D. …and the restart.
  await reboot();
  await step('D. after the restart');

  // E. Now the church removes the steward from the roster ENTIRELY, and reboots.
  console.log('remove OK =', await publishAs(church, doc(church, D.STEWARDS + church.pub, { pubkeys: [] })));
  await sleep(300);
  await step('E. steward removed from roster, NO restart');
  await reboot();
  await step('F. after the restart');

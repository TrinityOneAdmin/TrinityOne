      ...
      {givingOn && view === 'giving' ? (
        ...
        <div style={{ textAlign: 'center', padding: '40px 24px', animation: 'trinityFade .4s ease both' }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 20, padding: '10px 16px', borderRadius: 999, background: ctx.joinFailed ? 'color-mix(in oklab, var(--danger, #c0392b) 12%, var(--surface))' : 'color-mix(in oklab, var(--gold) 12%, var(--surface))', border: ctx.joinFailed ? '1px solid color-mix(in oklab, var(--danger, #c0392b) 30%, transparent)' : '1px solid color-mix(in oklab, var(--gold) 30%, transparent)', color: ctx.joinFailed ? 'var(--danger, #c0392b)' : '#8a6717', fontWeight: 700, fontSize: 13.5 }}>
            <span style={{ width: 8, height: 8, borderRadius: 999, background: ctx.joinFailed ? 'currentColor' : '#c2913a' }} /> {ctx.joinFailed ? 'Not sent — the church hasn’t seen this' : (ctx.joinQueued || ctx.joinSent) ? 'Pending steward approval' : 'Not sent yet — the church hasn’t seen this'}
          </div>
    ...
    <Overlay open={open} onClose={onClose} docked={docked}>
      ...
      <div ref={scRef} className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 8px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        {/* THE STATE, AND NOTHING ELSE. This was a full sentence — "Church room · not end-to-end encrypted, so
            ...
            (2026-08-13): a standing warning on a screen you open twenty times a day is not read, it is just
            noise, and it made the room look like a place to be careful rather than a place to talk.
            The pill still states the fact, which is the part that must not go: a room the relay can read says

...
function StewardWelcome() {
  ...
  const finishWithPin = async () => {
    if (pinVal.length < 8) { setErr('Use at least 8 characters. Best is one generated by a password manager; a passphrase of four random words works well too.'); return; }   // AUDIT-2026-07-30: raised 6→8 (six digits is ~10^6 — roughly half a minute on one GPU from a copied blob, even at PBKDF2-600k)
    setPinBusy(true);
    ...
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'radial-gradient(120% 80% at 50% -10%, var(--gold-tint, #f6edda), var(--paper))' }}>
      <div style={card}>
        ...
        {mode === 'choose' ? (
          ...
          <div>
            <div style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.55, marginBottom: 14 }}>Set a PIN to protect this device. Your steward key can act for a church, so it’s <b>encrypted on this device</b> and this unlocks it — a lost or shared phone can’t act as a church without it. Use at least 8 characters: a generated password from a password manager is best, or four random words you can picture together.</div>
            <input type="password" aria-label="Choose a console PIN or passphrase" value={pinVal} onChange={e => { setPinVal(e.target.value); setErr(''); }} autoFocus placeholder="At least 8 — a generated password is best" autoComplete="new-password" onKeyDown={e => { if (e.key === 'Enter' && pinVal.length >= 8 && !pinBusy) finishWithPin(); }} style={{ width: '100%', boxSizing: 'border-box', border: '1px solid var(--line)', borderRadius: 12, background: 'var(--surface-2)', padding: '12px 14px', fontSize: 15, fontFamily: 'var(--font-ui)', color: 'var(--ink)', outline: 'none', letterSpacing: '2px' }} />
          ...
          <div>
            ...
            <textarea value={phrase} onChange={e => setPhrase(e.target.value)} rows={3} aria-label="Your church's 12-word recovery phrase" placeholder="word one  word two  word three …" style={{ width: '100%', boxSizing: 'border-box', border: '1px solid var(--line)', borderRadius: 12, background: 'var(--surface-2)', padding: '11px 13px', fontSize: 13.5, fontFamily: 'var(--mono)', color: 'var(--ink)', outline: 'none', resize: 'vertical', lineHeight: 1.6 }} />
...
function StewardForcedPin() {
  ...
  const submit = async () => {
    ...
    if ((pin || '').length < 8) { setErr('Use at least 8 characters — a generated password, or a passphrase of four random words.'); return; }
    ...
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'radial-gradient(120% 80% at 50% -10%, var(--gold-tint, #f6edda), var(--paper))' }}>
      <div style={{ width: 'min(440px, 92vw)', background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 22, boxShadow: 'var(--shadow-lg)', padding: 28, textAlign: 'center' }}>
        ...
        <input type="password" aria-label="Choose a console PIN or passphrase" value={pin} autoFocus onChange={e => { setPin(e.target.value); setErr(''); }} onKeyDown={e => { if (e.key === 'Enter') submit(); }}
          placeholder="At least 8 — a generated password is best" autoComplete="new-password"
        ...
        <input type="password" aria-label="Repeat the console PIN or passphrase" value={pin2} onChange={e => { setPin2(e.target.value); setErr(''); }} onKeyDown={e => { if (e.key === 'Enter') submit(); }}
...
function StewardRoot() {
  ...
  // (A restored / adopted / "help run a church" identity is kept OUT of the setup wizard by the restore paths
  // themselves, which set `trinityone.steward.wizard.done` — StewDashboard gates its first-run wizard on that
  // flag directly. This used to need a second nudge here to flip consoleView away from the old StewWizard;
  // with that wizard gone the nudge had nothing to set, and its call survived only because the surrounding
  // try/catch silently swallowed the ReferenceError. Removed rather than left lying.)

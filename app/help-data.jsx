...
window.HelpData = {
  ...
  articles: [
    ...
    // clearance is and is not, and what a session key costs when it is replaced.
    {
      ...
      minutes: 3,
      blocks: [
        { type: 'p', text: 'Check-in is a door operation. A leader stands at the children’s room with this console open, checks each child in, and gives the parent the pickup code. At collection you ask for the code, match it against the one on screen, and check the child out. Nobody has to have an app for any of it. The register lists who is still in the room rather than who arrived today, so an evening session that runs past midnight, or an overnight event, keeps everybody on it; a row that started on an earlier day says which day beside the arrival time, and one that is never checked out drops off on its own within about a day.' },

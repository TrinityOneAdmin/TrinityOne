...
function App() {
  ...
  const ctx = {
    ...
    care: {
      ...
      fill: (careId, iso, note) => { setOptCare(o => ({ ...o, [careId + '|' + iso]: 'fill' })); return window.Fellowship.fillCareSlot(careId, iso, note).then(r => { if (r && r.ok) toast('Thank you — you’re signed up'); else if (r && r.reason === 'unconfirmed') toast('We couldn’t confirm that reached your church — it may well have. You are shown as helping; check the row again in a moment rather than signing up a second time.', { error: true }); else { setOptCare(o => { const n = { ...o }; delete n[careId + '|' + iso]; return n; }); toast('That didn’t reach your church — you’re NOT signed up. Try again in a moment.', { error: true }); } return r; }); },
      clearFill: (careId, iso) => { setOptCare(o => ({ ...o, [careId + '|' + iso]: 'clear' })); return window.Fellowship.clearCareSlot(careId, iso).then(r => { if (r && r.ok) toast('Removed'); else if (r && r.reason === 'unconfirmed') toast('We couldn’t confirm that reached your church — it may well have. You are shown as no longer helping; check the row again in a moment rather than changing it back.', { error: true }); else { setOptCare(o => { const n = { ...o }; delete n[careId + '|' + iso]; return n; }); toast('That didn’t reach your church — you’re still down for that day.', { error: true }); } return r; }); },

...
BOOKS = {
    ...
    "inhissteps":  {"name": "In His Steps", "author": "Charles M. Sheldon", "year": "1896", "kw": "in his steps", "gids": [4540]},
    "benhur":      {"name": "Ben-Hur: A Tale of the Christ", "author": "Lew Wallace", "year": "1880", "kw": "ben-hur", "gids": [2145]},
}
...
def load_existing():
    """Return (available, previews) from the current index.js, so an incremental build (ids passed
    as args) preserves the books it isn't re-fetching instead of dropping them."""
    path = os.path.join(OUT, "index.js")
    try:
        txt = open(path, encoding="utf-8").read()
        m = re.search(r"window\.TrinityLibrary\s*=\s*(\{.*\});\s*$", txt, re.S)
...
def main():
    ...
    # optional args = build ONLY these ids and merge into the existing catalog (keeps vetted books)
    only = [a for a in sys.argv[1:] if a in BOOKS]
    if only:
        available, previews = load_existing()
        todo = {bid: BOOKS[bid] for bid in only}
        print(f"incremental build: {', '.join(only)} (merging into {len(available)} existing)")
    else:
        available, previews = [], {}
        todo = BOOKS
    for bid, meta in todo.items():
        ...
        if bid not in available: available.append(bid)

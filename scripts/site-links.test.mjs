...
// welcome-sim.html is a SIM ARTEFACT: a copy of welcome.html whose only difference is that its links point
// at the local machine instead of app.trinityone.church, so a round can start where a real person starts.
// Gitignored, never deployed, and deliberately unlinked — same category as apks.html.
const NOT_TRACKED = new Set(['apks.html', 'welcome-sim.html']);

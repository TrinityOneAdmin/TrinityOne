...
const GROUPKEY_D = D.GROUPKEY;   // ONE ENCRYPTED ROOM's key, wrapped per member — d=groupkey:<groupId>. Scoped to the church that owns the GROUP, never to a ['church'] tag: publishGroupKey writes none.
...
function accept(e) {
  ...
  if (k === 30078) {
    ...
    //
    // THE OWNING CHURCH IS THE GROUP'S, NOT THE WRITER'S. Nothing trusts a ['church'] tag here and there is
    // not one to trust: publishGroupKey signs `[['d', ...], ['t', NET]]` and never goes through feChurch, so
    // namedChurch(e) is '' for every envelope a delegated console has ever written. Resolution mirrors

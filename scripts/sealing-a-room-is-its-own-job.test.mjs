...
// An UNTAGGED envelope, which is what every console in the field writes today and what publishGroupKey wrote
// when this file was first written. ⚠ IT IS NO LONGER THE ONLY SHAPE: since the 2026-09-17 merge with
// fix/delegated-steward-writes-2026-09-16, publishGroupKey goes through feChurch, so a delegated console's
// envelope DOES carry ['church', <cp>] — without it the relay stored the key and served it to nobody.
// Both shapes are deliberately exercised here, and neither may decide the owning church:
//   · untagged (this builder) — the field case, and the case a tag-reading rule would simply have no answer for
//   · tagged with someone ELSE'S church — the forgery, three tests below ("naming your own church in a
//     ['church'] tag does not move the room to it"), which is what the tag being writer-chosen actually costs
//   · tagged with THIS church, written by the SHIPPED publishGroupKey — the honest case, driven end to end in
//     scripts/a-sealed-room-reaches-the-church-that-owns-it.test.mjs, where the tag is what makes the READ work
let keyAt = now();

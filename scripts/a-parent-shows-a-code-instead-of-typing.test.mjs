...
function fellowship(ls, clock) {
  const scope = {
    localStorage: ls, _mePub: () => ME, _mayCache: () => true, toPub: (x) => String(x || ''),
...
test('REFUSED is settled, and it sends them to the desk without blaming anyone', async () => {
  // The commonest cause: the church has not opened a children's room for this service at all. The steward's
  // "children's work runs here" tick does not exist yet (no console service editor), so the card cannot know
  // in advance — the relay's refusal is the honest answer and this is where it is worded.

...
export const DOC_TYPES = Object.freeze({
  ...
  'trinityone/checkin:':      { write: 'mixed',     read: 'church',  scope: 'tag',    note: 'one child\u2019s presence at one session, sealed under the SAFEGUARDING key. Church or a steward holding safeguarding; NOT the member catch-all — these are addressable, so an ordinary member could otherwise overwrite a child\u2019s presence record with anything and it would vanish from the register. READ WAS \'members\' UNTIL 2026-09-09 \u2014 every member of the church received the ciphertext, and this registry said so outright. The check-in helper capability made that indefensible: a helper key is scoped to one session, and a register whose ciphertext the whole congregation already holds cannot be re-scoped by any gate afterwards. The set is now the church, its network, a steward holding safeguarding, an IN-WINDOW HELPER of the session named in the record\u2019s [\'session\'] tag, and the guardian the record names in a [\'p\'] tag. WRITE gained that same helper. The column says \'church\' because there is no value for this set and \'members\' would now be a lie; the honest list is the sentence above' },
  // \u2500\u2500 THE CHECK-IN HELPER CAPABILITY, 2026-09-09 \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
  'trinityone/checkinhelper:': { write: 'church',  read: 'church',  scope: 'author', note: 'ONE SESSION\u2019S check-in helpers, and that session\u2019s key wrapped to each of them \u2014 d=checkinhelper:<serviceId>. OWNER-ONLY mint, for the same reason checkinkey: is owner-only: whoever writes this decides who may read a child\u2019s pickup code, and a safeguarding steward being able to hand that to a third party is an escalation they do not have today. CLEARTEXT and deliberately so \u2014 {session, source, lifetime, from, until, pubs:[\u2026], keys:{<pub>:wrapped}} \u2014 because the relay has to read the window and the pubkey list it is being asked to enforce, exactly as it reads rota-settings and roster:\u2019s cleartext pubs. It names NO child and carries no child data. `source` records WHICH question answered \'who may hold this key\' (scripts/checkin-role-source.mjs \u2014 the rota today, a named safeguarding team later) and the relay refuses a source it does not declare, so the swap point is in the enforced record rather than in a comment. `lifetime` records how long the church chose to let it last (session / day / until a steward ends it \u2014 owner\u2019s decision 2026-09-09, and the default is the tightest). The WINDOW is what makes that enforceable rather than assumed: the relay compares it against ITS OWN clock, never the event\u2019s created_at, and the cap travels with the declared lifetime so a \'session\' grant cannot be handed a week-long window by any client. Only the lifetime whose entire point is \'until a steward ends it\' may omit an end, and a tombstone revokes any of them at once. NOT MEMBER-READABLE, and the column said \u2018members\u2019 until 2026-09-09 because it was copied from the other key envelopes without asking what THIS one carries. Measured on a live gateway: an ordinary member correctly refused `rota:` under `rota-settings` visibility \u2018stewards\u2019 was still served this document, whose cleartext `pubs` array names everyone rostered to children\u2019s work that morning \u2014 silently reversing the decision the church had just made. The honest read set is the church, its network, any steward, a care admin, and A PUBKEY THE GRANT ITSELF NAMES (who may not be a member of the congregation at all, which is why \u2018members\u2019 was never the right word in either direction). The envelope stays safe to hand a named helper for the usual reason \u2014 each recipient can only unwrap their own slot and the relay never sees the session key \u2014 but that is an argument for giving it to the PEOPLE IT NAMES, not to everyone. The window is deliberately not applied to this read: a helper fetching their key ten minutes early is how a Sunday starts, and the key is useless without the records, which ARE window-gated' },

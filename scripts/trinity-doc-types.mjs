...
export const DOC_TYPES = Object.freeze({
  ...
  // THE SEALED-ROOM KEY, given a rule of its own on 2026-09-17. It was UNDECLARED until then — the entry that
  // used to sit in UNDECLARED below has been removed, because it is declared now, and what it cost is written
  // here instead. With no branch in accept() it fell to the generic member catch-all, which answers the
  // relay-wide question "is this key a member of ANY church on this box" — so a steward of THIS church who
  // had not also joined it was REFUSED her own room's key, while a member of a CO-TENANT church was ACCEPTED
  // writing it. Both measured on a live relay.
  //
  // TWO DIFFERENT QUESTIONS ASK TWO DIFFERENT THINGS, and the split is the point of this entry.
  //   · WRITE (accept()) resolves the owning church from the GROUP the d-tag names — GROUP_CHURCH, then the
  //     owner the id itself spells — and NEVER from the ['church'] tag on the event. That is not because the
  //     tag is absent: since the delegated-steward fix of 2026-09-16, publishGroupKey goes through feChurch
  //     and every envelope DOES carry one. It is because the tag is written by whoever signs the envelope, so
  //     trusting it would let a steward of church A stamp church B's name on a key and have it admitted — the
  //     cross-church hole this rule was written to close. A scoped sabotage in
  //     scripts/sealing-a-room-is-its-own-job.test.mjs swaps the resolution for the tag and proves it reopens.
  //   · READ (canRead) is the opposite, and needs that same tag to exist: it attributes a kind-30078 to a
  //     church so the church and its members can be served it. Before feChurch the envelope named no church,
  //     so a delegated steward's key was stored and then served to nobody — the room rendered EMPTY, not
  //     broken. `scope: 'tag'` in the column below is the READ side.

  ...
  (function() {
    ...
    function parseDate(s) {
      ...
      if (m = s.match(/^(\d{1,2})[\/\-.](\d{1,2})[\/\-.](\d{2,4})/)) {
        let d2 = m[1].padStart(2, "0"), mo = m[2].padStart(2, "0"), y = m[3];
    ...
    function buildRows(parsed, mapping) {
      ...
      for (const r of parsed.rows || []) {
        const date = parseDate(r[mapping.date] || "") || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
    ...
    function saveDonor(d) {
      ...
      const rec = {
        ...
        lastName,
        house: (d.house || "").trim(),
        // HMRC: house name or number (taken from the address)
        ...
        giftAid: d.giftAid && d.giftAid.declared ? { declared: true, date: d.giftAid.date || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10) } : { declared: false },
        note: (d.note || "").trim()
      };
      return S().encPublish(DONOR_D + id, rec).then(() => ({ id, ...rec }));
    }
    function removeDonor(id) {
      return S() && S().encRemove ? S().encRemove(DONOR_D + id) : Promise.resolve(null);
    }
    ...
    function saveTx(t) {
      ...
      const rec = {
        ...
        // set on bank-statement import to de-dupe re-imports of the same line
        claimId: t.claimId || ""
        // set when this donation has been included in a Gift Aid claim (won't be re-claimed)
      };
      return S().encPublish(TX_D + id, rec).then(() => ({ id, ...rec }));
    }
    function removeTx(id) {
      return S() && S().encRemove ? S().encRemove(TX_D + id) : Promise.resolve(null);
    }
    function giftAidSummary(txs, donors) {
      ...
      for (const t of txs || []) {
        if (t.giftAid && (t.currency || "GBP") === "GBP" && t.donorId && decl.has(t.donorId)) {
          eligible += num(t.amount);
          count++;
        }
      }
    ...
    function exportLedgerCsv(txs, donors, funds) {
      ...
      const rows = (txs || []).map((t) => [
        t.date,
        dById[t.donorId] && dById[t.donorId].name || "Anonymous",
        num(t.amount).toFixed(2),
        t.currency || "GBP",
        t.method || "",
        fById[t.fundId] && fById[t.fundId].name || "",
        t.giftAid ? "Yes" : "No",
        t.note || ""
      ]);
    ...
    function exportGiftAidCsv(txs, donors) {
      ...
      const rows = (txs || []).filter((t) => t.giftAid && (t.currency || "GBP") === "GBP" && t.donorId && decl.has(t.donorId)).map((t) => {
        const d = dById[t.donorId] || {};
        return [d.name || "", d.address || "", d.postcode || "", t.date, num(t.amount).toFixed(2)];
      });
      return csv([head, ...rows]);
    }
    function exportStatementsHtml(year, txs, donors, funds, churchName) {
      ...
      const money = (n) => "\xA3" + num(n).toFixed(2);
      const yr = String(year);
      const byDonor = /* @__PURE__ */ new Map();
      for (const t of txs || []) {
      ...
      const sections = [...byDonor.entries()].map(([id, gifts]) => ({ d: dById[id] || { name: "Donor" }, gifts: gifts.slice().sort((a, b) => String(a.date).localeCompare(String(b.date))) })).sort((a, b) => (a.d.name || "").localeCompare(b.d.name || "")).map(({ d, gifts }) => {
        ...
        return `<section class="stmt"><h2>${esc(churchName || "Our Church")}</h2><div class="sub">Giving statement \xB7 ${esc(yr)}</div><div class="to"><b>${esc(d.name || "")}</b>${d.address ? "<br>" + esc(d.address) : ""}${d.postcode ? "<br>" + esc(d.postcode) : ""}</div><table><thead><tr><th>Date</th><th>Fund</th><th>Method</th><th class="r">Amount</th></tr></thead><tbody>${rows}</tbody><tfoot><tr><td colspan="3"><b>Total for ${esc(yr)}</b></td><td class="r"><b>${money(total)}</b></td></tr></tfoot></table>` + (d.giftAid && d.giftAid.declared ? `<p class="ga">Gift Aid declaration on file${d.giftAid.date ? " (since " + esc(d.giftAid.date) + ")" : ""}. Gift-Aided giving this year: <b>${money(ga)}</b>.</p>` : `<p class="ga muted">No Gift Aid declaration on file.</p>`) + `<p class="thx">Thank you for your generous giving. Please keep this statement for your records.</p></section>`;
      }).join("");
      return `<!doctype html><html><head><meta charset="utf-8"><title>Giving statements ${esc(yr)}</title><style>body{font-family:Georgia,'Times New Roman',serif;color:#1b1714;max-width:720px;margin:0 auto;padding:24px}.bar{font-family:system-ui,sans-serif;background:#f4efe6;border:1px solid #e6ddcb;border-radius:10px;padding:12px 14px;margin-bottom:20px;font-size:13px;color:#5b5345;display:flex;align-items:center;gap:12px}.bar button{font:inherit;background:#8a6717;color:#fff;border:none;border-radius:8px;padding:7px 13px;cursor:pointer;font-weight:700}.stmt{page-break-after:always;border-bottom:1px dashed #ccc;padding-bottom:28px;margin-bottom:28px}.stmt:last-child{border:none}h2{margin:0;font-size:22px}.sub{color:#777;font-size:13px;font-family:system-ui,sans-serif}.to{margin:14px 0 10px;font-size:15px;line-height:1.5}table{width:100%;border-collapse:collapse;font-size:14px;font-family:system-ui,sans-serif}th,td{text-align:left;padding:6px 8px;border-bottom:1px solid #eee}.r{text-align:right}tfoot td{border-top:2px solid #333;border-bottom:none}.ga{font-family:system-ui,sans-serif;font-size:13px;color:#345c41}.muted{color:#999}.thx{font-style:italic;color:#555;font-size:13.5px}@media print{.bar{display:none}}</style></head><body><div class="bar"><span>${byDonor.size} statement${byDonor.size === 1 ? "" : "s"} for ${esc(yr)}.</span><button onclick="window.print()">Print / Save as PDF</button></div>` + (sections || `<p>No donor-attributed giving recorded for ${esc(yr)}.</p>`) + `</body></html>`;
    }
    ...
    function eligibleForClaim(txs, donors, range) {
      ...
      return (txs || []).filter((t) => t.giftAid && (t.currency || "GBP") === "GBP" && t.donorId && !t.claimId && dById[t.donorId] && dById[t.donorId].giftAid && dById[t.donorId].giftAid.declared).filter((t) => (!from || t.date >= from) && (!to || t.date <= to)).map((t) => ({ tx: t, donor: dById[t.donorId] })).sort((a, b) => (a.tx.date || "").localeCompare(b.tx.date || ""));
    }
    ...
    function claimSummary(eligible) {
      let total = 0, bad = 0;
      const donors = /* @__PURE__ */ new Set();
      for (const { donor, tx } of eligible) {
        total += num(tx.amount);
        donors.add(donor.id);
        if (validateClaimRow(donor, tx).length) bad++;
      }
    ...
    async function recordClaim(eligible, range) {
      ...
      await S().encPublish(CLAIM_D + id, { date: (/* @__PURE__ */ new Date()).toISOString().slice(0, 10), from: range && range.from || "", to: range && range.to || "", total: sum.total, reclaimable: sum.reclaimable, donations: sum.donations, donors: sum.donors });

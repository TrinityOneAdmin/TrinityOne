(() => {
  ...
  var _applying = /* @__PURE__ */ new Set();
  var LISTHW_KEY = "trinityone.relaylist.hw";
  function _loadHW(cp) {
    try {
      const v = JSON.parse(localStorage.getItem(LISTHW_KEY) || "{}")[cp];
      return typeof v === "number" && isFinite(v) ? v : 0;
    } catch {
      return 0;
    }
  }
  function _saveHW(cp, at) {
    try {
      const m = JSON.parse(localStorage.getItem(LISTHW_KEY) || "{}");
      if (typeof m[cp] !== "number" || at > m[cp]) {
        m[cp] = at;
  ...
  async function _applyChurchList(cp) {
    if (_applying.has(cp)) return;
    _applying.add(cp);
    try {
      const cur = _churchList.get(cp);
      if (!cur) return;
      const at = cur.at, want = cur.want;
      await Promise.all([...want].map(async (u) => {
        let info = null;
        try {
          info = await _relayInfo(u);
        } catch {
        }
        if ((_churchList.get(cp) || {}).at !== at) return;
  ...
  var UNAVAIL_MIRROR = "trinityone.unavail.";
  var _profilePubFor = "";
  var _profilePubBody = "";
  ...
  var FUTURE_SKEW = 900;
  var FULL_SYNC_S = 86400;
  var HUB_SAVE_CAP = 3e6;
  var DOCSHUB_KEY = "trinityone.docshub.";
  var MEMHUB_KEY = "trinityone.memhub.";
  var _dtag = (e) => (e.tags.find((t) => t[0] === "d") || [])[1] || "";
  var _slimEvt = (e) => ({ id: e.id, pubkey: e.pubkey, created_at: e.created_at, kind: e.kind, tags: e.tags, content: e.content });
  var _hubCursor = (hub, e) => {
    const t = e.created_at || 0;
    if (t > hub.since && t <= Math.floor(Date.now() / 1e3) + FUTURE_SKEW) {
      hub.since = t;
      hub.dirty = true;
    }
  };
  ...
  function _sealChurchDocMember(cp, obj) {
    ...
    if (!k) return body;
    try {
      return JSON.stringify({ e: encrypt(body, k) });
    } catch (e) {
      return body;
    }
  }
  ...
  function _noteAdmitted(cp, content) {
    ...
    setTimeout(() => {
      ...
      try {
        window.dispatchEvent(new CustomEvent("trinity-admitted", { detail: { church: cp } }));
      } catch (e) {
      }
      try {
  ...
  function _docsHubSaveSoon(hub) {
    if (!hub.saveT && hub.dirty) hub.saveT = setTimeout(() => {
      hub.saveT = null;
      _docsHubSaveNow(hub);
    }, 800);
  }
  ...
  function _memHub(cp) {
    ...
    if (hub) return hub;
    hub = { cp, byPub: /* @__PURE__ */ new Map(), listeners: /* @__PURE__ */ new Set(), refs: 0, eosed: false, since: 0, fullAt: 0, pendingFull: false, dirty: false, saveT: null, closer: null };
    _memHubs.set(cp, hub);
    let seeded = false;
    try {
      ...
      if (raw && Array.isArray(raw.members)) {
        ...
        for (const m of raw.members) {
          if (m && m.pubkey) hub.byPub.set(m.pubkey, m);
        }
        seeded = true;
      }
    } catch {
  ...
  function _memHubOpen(hub) {
    ...
    const sub = pool.subscribeMany(relaysForChurch(cp), filters, {
      ...
      onevent(e) {
        ...
        hub.byPub.set(e.pubkey, m);
        hub.dirty = true;
        _memHubSaveSoon(hub);
        for (const l of [...hub.listeners]) {
          try {
            l.onchange && l.onchange(e.pubkey);
          } catch (err) {
            console.error(err);
          }
        }
  ...
  window.Fellowship = {
    ...
    async fetchBlob(url, opts) {
      ...
      if (typeof opts.decrypt === "function") bytes = await opts.decrypt(bytes);
    ...
    _dmWrap(text, replyTo) {
      ...
      try {
        return JSON.stringify({ v: 1, t: String(text), r: String(replyTo.id).slice(0, 64) });
      } catch (e) {
        return text;
      }
    },
    ...
    async sendDM(peerPub, content, replyTo) {
      ...
      let ciphertext;
      try {
        ciphertext = _dmEncrypt(sk, peerPub, body);
      } catch (e) {
    ...
    async setUnavailable(churchNpub, dates) {
      if (!sk) await window.Fellowship.ready;
      const cp = toPub(churchNpub);
      if (!cp || !sk) throw new Error("not ready");
      const me = window.Fellowship.myPubkey;
    ...
    getUnavailable(churchNpub) {
      ...
      if (!cp) return [];
      try {
        const v = JSON.parse(localStorage.getItem(UNAVAIL_MIRROR + cp) || "[]");
    ...
    subscribeChurchRelays(churchNpub) {
      ...
      const sub = pool.subscribeMany(churchRelays(), [{ kinds: [10002], authors: [cp] }], {
        onevent(e) {
          if (e.pubkey !== cp) return;
          const at = e.created_at || 0;
          if (at > Math.floor(Date.now() / 1e3) + 600) return;
      ...
      return () => {
        try {
          if (typeof window !== "undefined") window.removeEventListener("trinity-relays", onchurn);
        } catch {
        }
        try {
          sub.close();

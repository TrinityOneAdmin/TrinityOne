  ...
  window.Fellowship = {
    ...
    async publishMessage(groupId, content, extraTags = []) {
      ...
      const evt = finalizeEvent2({
        kind: 1,
        created_at: Math.floor(Date.now() / 1e3),
        tags: [["t", NET], ["t", groupId], ...churchTag, ...extraTags],
        content
      }, sk);
      try {
        await Promise.any(pool.publish(window.Fellowship.relays, evt));
    ...
    async sendDM(peerPub, content) {
      ...
      let ciphertext;
      try {
        ciphertext = await encrypt2(sk, peerPub, content);
      } catch (e) {
    ...
    subscribeThread(peerPub, onMsg) {
      ...
      const deliver = async (e) => {
        ...
        let content = "";
        try {
          content = await decrypt2(sk, peerPub, e.content);
        } catch (err) {
          content = "\u{1F512} (could not decrypt)";
        }
        try {
          onMsg({ id: e.id, mine, content, ts: e.created_at, pubkey: e.pubkey });
        } catch (err) {
        }
      };
    ...
    subscribeDMs(onConvos) {
      ...
      const handle = async (e) => {
        ...
        let preview = "";
        try {
          preview = await decrypt2(sk, peer, e.content);
        } catch (err) {
          preview = "\u{1F512}";
        }
    ...
    subscribeChurchServices(churchNpub, cb) {
      return window.Fellowship._subChurchAddr(churchNpub, "trinityone/service:", (c) => ({ date: c.date, time: c.time, name: c.name }), cb);
    },
    ...
    subscribeChurchEvents(churchNpub, cb) {
      return window.Fellowship._subChurchAddr(churchNpub, "trinityone/event:", (c) => ({ date: c.date, time: c.time, title: c.title, where: c.where, blurb: c.blurb, accent: c.accent }), cb);
    },

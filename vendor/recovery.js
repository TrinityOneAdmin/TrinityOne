(() => {
  ...
  function evalPoly(coeffs, x) {
    let acc = 0;
    for (let j = coeffs.length - 1; j >= 0; j--) acc = gmul(acc, x) ^ coeffs[j];
    return acc;
  }
  function splitBytes(secret, threshold, total, rand) {
  ...
  var _hex = (u82) => Array.from(u82, (b) => b.toString(16).padStart(2, "0")).join("");
  var _unhex = (s) => {
  ...
  function crc16(str) {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      for (let k = 0; k < 8; k++) c = c & 32768 ? (c << 1 ^ 4129) & 65535 : c << 1 & 65535;
    }
  ...
  function splitMnemonic(mnemonic, opts = {}) {
    const threshold = opts.threshold || 2, total = opts.total || 3;
    const rand = opts.rand || ((n) => {
  ...
  function combineMnemonic(shareStrings) {
  ...
  // src/recovery.src.js
  var ARGON2_DEFAULT = { t: 2, m: 19456, p: 1 };

...
zoo`.split("\n");
  ...
  window.Steward = {
    ...
    publishProfile(meta) {
      ...
      const content = JSON.stringify({ name: m.name || "", about: m.about || "", nip05, picture: m.picture || "", banner: m.banner || "", accent: m.accent || "", channel: m.channel || "", audioFeed: m.audioFeed || "", lud16: (m.lud16 || "").trim(), giving: !!m.giving });
    ...
    removeFund(id) {
      if (!sk) return Promise.resolve(null);
      return publish(finalizeEvent2({ kind: 30078, created_at: now(), tags: [["d", FUND_D + id], ["t", NET], ["deleted", "1"]], content: "" }, sk));
    },
    ...
    subscribeGroupChat(groupId, onMsgs) {
      ...
      const attach = () => [...byId.values()].sort((a, b) => (a.ts || 0) - (b.ts || 0)).map((m) => {
        const r = rx.get(m.id);
        return { ...m, reactions: r ? [...r.values()].filter(Boolean) : [], myReaction: r ? r.get(pub) || "" : "" };
      });
    ...
    publishRoster(teamId, roster) {
      ...
      return publish(finalizeEvent2({ kind: 30078, created_at: now(), tags: [["d", ROSTER_D + teamId], ["t", NET]], content }, sk)).then(() => ({ id: teamId, roles, people }));
    },
    subscribeRosters(onRosters) {
      return this._subAddr(ROSTER_D, (c, id) => ({ team: id, roles: c.roles || [], people: c.people || [] }), onRosters);
    },
    ...
    publishService(svc) {
      ...
      return publish(finalizeEvent2({ kind: 30078, created_at: now(), tags: [["d", SERVICE_D + id], ["t", NET]], content }, sk)).then(() => ({ id, ...JSON.parse(content) }));
    },
    removeService(id) {
      if (!sk) return Promise.resolve(null);
      return publish(finalizeEvent2({ kind: 30078, created_at: now(), tags: [["d", SERVICE_D + id], ["t", NET], ["deleted", "1"]], content: "" }, sk));
    },
    ...
    publishRota(rota) {
      ...
      return publish(finalizeEvent2({ kind: 30078, created_at: now(), tags: [["d", ROTA_D + rota.service], ["t", NET]], content }, sk)).then(() => ({ id: rota.service, service: rota.service, published: !!rota.published, assign: rota.assign || {} }));
    },
    removeRota(serviceId) {
      if (!sk) return Promise.resolve(null);
      return publish(finalizeEvent2({ kind: 30078, created_at: now(), tags: [["d", ROTA_D + serviceId], ["t", NET], ["deleted", "1"]], content: "" }, sk));
    },
    ...
    publishEvent(ev, asPub) {
      ...
      return publish(finalizeEvent2({ kind: 30078, created_at: now(), tags, content }, signer)).then(() => ({ id, ...JSON.parse(content) }));
    },
    removeEvent(id) {
      if (!sk) return Promise.resolve(null);
      return publish(finalizeEvent2({ kind: 30078, created_at: now(), tags: [["d", EVENT_D + id], ["t", NET], ["deleted", "1"]], content: "" }, sk));
    },
        ...
        onevent(e) {
          ...
          if (e.kind === 30078) {
            ...
            if (d.startsWith(GROUP_D)) {
              ...
              item = { ic: "chat", tint: "sage", text: deleted ? "A group was removed" : `Group \u201C${n || "untitled"}\u201D ${own ? "created" : "updated"}` };
            } else if (d.startsWith("trinityone/member:")) {
              if (!deleted) item = { ic: "pray", tint: "sage", text: "A new member joined" };
              ...
              item = { ic: "gift", tint: "gold", text: deleted ? "A fund was removed" : `Fund \u201C${n || ""}\u201D updated` };
            }
          } else if (e.kind === 1) {
            if (own) item = { ic: "send", tint: "gold", text: "You posted an announcement" };
            else {
              const g = (e.tags.find((t) => t[0] === "t" && t[1] !== NET) || [])[1] || "a group";
              item = { ic: "chat", tint: "clay", text: `New message in ${g}` };
            }
          }
          if (item) {

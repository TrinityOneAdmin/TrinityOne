...
zoo`.split("\n");
  ...
  window.Steward = {
    ...
    publishPost(content, group) {
      if (!sk) return Promise.resolve(null);
      return publish(finalizeEvent2({ kind: 1, created_at: now(), tags: [["t", NET], ["t", group || "announce"]], content: content || "" }, sk));
    },
    ...
    publishEvent(ev) {
      ...
      return publish(finalizeEvent2({ kind: 30078, created_at: now(), tags: [["d", EVENT_D + id], ["t", NET]], content }, sk)).then(() => ({ id, ...JSON.parse(content) }));
    },
    removeEvent(id) {
      if (!sk) return Promise.resolve(null);

...
// HOW FAR BACK — AND FORWARD — A PARENT'S OWN SCREEN LOOKS. See subscribeMyChildrenCheckins for why this is
// load-bearing rather than tidiness: A GUARDIAN IS NEVER SERVED A TOMBSTONE, so a record with no release to
// fold onto it would say a child is in a room for ever.
//
// ⚠ IT IS MAX_SESSION_SECONDS, NOT A ROUND NUMBER SOMEBODY LIKED. 16 hours was the first figure here and an
// audit measured what it cost: `HELPER_LIFETIMES.day.max`, `PERMISSION_LIFETIMES.day.max` and
// MAX_SESSION_SECONDS are all 26 hours, precisely so a church can run "a morning and an afternoon, or an
// all-day event" off one session. At 16 hours a lock-in or a residential lost the parent's row WHILE THE
...
window.Fellowship = {
  ...
  subscribeCheckinRegister(churchNpub, cb) {
    ...
    const emit = _coalesce(() => {
      ...
      // slice C / KNOT 2: a checkout is a SEPARATE document (a helper cannot rewrite the church's record —
      // F-B), carrying a cleartext ['rel', <checkinId>] tag. Folded onto the check-in row it names, and ONLY
      // when the release's own session matches the check-in's — both cleartext, so a release scoped to one
      // session can never mark a child collected in another (the F1 shape, one level up). Keyed by
      // session|checkinId so the match is structural.
      ...
      for (const [id, r] of recs) {
        ...
        // A RELEASE, routed by its CLEARTEXT rel tag (no key needed to tell it from a child) — folded, never
        // rendered as its own row, so it can never appear as a phantom child.
  ...
  setArrivalOutcome(churchNpub, session, res) {
    ...
    // `_mayCache()` for the same reason setBringsChildren has it — and here it costs nothing at all:
    // `writeArrival` refuses with `no-identity` on a locked phone, so there is never an outcome to lose.
    if (!slot || !sid || !_mayCache()) return false;
    try {
      localStorage.setItem(slot, JSON.stringify({
        session: sid, ok: !!(res && res.ok), reason: String((res && res.reason) || ''), at: Math.floor(Date.now() / 1000),
      }));
    } catch (e) { return false; }   // quota, private mode, site data blocked — the card keeps its own state
    return true;
  },
  ...
  async releaseCheckin(churchNpub, rec) {
    ...
    if (!sid) return { ok: false, reason: 'no-session' };
    if (!rel) return { ok: false, reason: 'no-rel' };                // a release must name the check-in it collects
    const keyHex = _ckMemKeyGet(cp, sid);
  ...
  subscribeMyChildrenCheckins(churchNpub, cb) {
    ...
    const openRec = (id, r) => {
      ...
      const _str = (v) => (typeof v === 'string' ? v : (typeof v === 'number' && Number.isFinite(v) ? String(v) : ''));
      const _when = (v) => (typeof v === 'number' && Number.isFinite(v) ? v : (typeof v === 'string' && /^\d{1,12}$/.test(v) ? Number(v) : undefined));
      // `_by` IS CARRIED AND NOT USED, exactly as subscribeCheckinRegister carries it and for the same
      // reason: this reader does not filter by author (see the note above on what that costs and why a
      // client-side filter would hide the ordinary worker-written record), so the author travels with the
      // row and the fix has somewhere to land the day the relay gets one.
      rows.set(id, { id, session: r.sid, ts: r.ts, _by: r.by,
        childName: _str(obj.childName), code: _str(obj.code), in: _when(obj.in), out: _when(obj.out),
        rel: _str(obj.rel), manual: obj.manual === true });
      return 'ok';
    };
